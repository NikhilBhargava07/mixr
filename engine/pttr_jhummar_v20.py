...
v19 — THE DRIFT IS GONE. beat_this (transformer downbeat tracker) gives
per-bar downbeats instead of one global tempo. Comparing my fixed grid against
its cleaned output exposed the exact fault the dancer heard:

    my bar 16  -4ms      my bar 20  -63ms
    my bar 17 -19ms      my bar 21  -78ms
    my bar 18 -14ms      my bar 22  -73ms
    my bar 19 -48ms  <-- 2nd "she a rebel" (48.3s)   my bar 23 -108ms

My fixed bar was 2.4149s; the true bar is 2.4000s. A 15ms/bar error accumulates
and crosses the ~50ms audibility threshold precisely at bar 19 — which is where
the dancer said "on time perfectly up until the second rebel iteration".

Fix: every bed event is now placed relative to the ACTUAL detected downbeat of
its bar, with slots interpolated across that bar's real length. No global tempo
assumption, so no accumulation.

v20 — v19's BUG: the bed was placed on real downbeats (bar 0 = DB[_I0] =
PTTR 5.22s) but load() still fetched PTTR audio from the OLD fixed-grid
position (4.98s). So the bed and the audio disagreed by 240ms — the bed was
right, the audio was 240ms out from under it. That is why v19 sounded worse
than v18 despite better placement. Fixed: PTTR is now loaded from the same
real downbeats the bed uses, so render bar N == DB[_I0 + N] for both.

Original grid note: native 99.38 BPM, no stretch, downbeat 2.5658s from the
...
OUT = ROOT / "renders"
_DBP = ROOT / "workdir" / "pttr_downbeats.npy"
DOWNBEATS = np.load(_DBP) if _DBP.exists() else None
...
def clean_downbeats():
    """beat_this emits a few spurious downbeats (bars as short as 0.08s).
    Drop anything closer than 1.5s to its predecessor."""
    if DOWNBEATS is None:
        return None
    keep = [DOWNBEATS[0]]
    for x in DOWNBEATS[1:]:
        if x - keep[-1] > 1.5:
            keep.append(x)
...
DB = clean_downbeats()
# our bar 0 = the PTTR downbeat nearest the lyric-anchored start
_START = DOWNBEAT + (BITCH_BAR - INTRO_BARS) * PTTR_BAR
_I0 = int(np.argmin(np.abs(DB - _START))) if DB is not None else 0


def real_bar_time(bar_idx):
    """Start time of our bar N, taken from the ACTUAL detected downbeat."""
    if DB is None:
        return bar_idx * (4 * 60.0 / BPM)
    i = min(_I0 + bar_idx, len(DB) - 1)
    return float(DB[i] - DB[_I0])


def real_bar_len(bar_idx):
    if DB is None:
        return 4 * 60.0 / BPM
    i = min(_I0 + bar_idx, len(DB) - 2)
    return float(DB[i + 1] - DB[i])


def slot_at(bar_idx, slot, swing=SWING, slots=4):
    """Time of a 16th slot inside a bar of its REAL measured length."""
    L = real_bar_len(bar_idx)
    beat, sub = divmod(slot, slots)
    p = sub / slots
    if swing != 0.5:
        p = p * 2 * swing if p < 0.5 else swing + (p - 0.5) * 2 * (1 - swing)
    return real_bar_time(bar_idx) + (beat + p) * (L / 4.0)


def load_bars_db(bar_from, n_bars):
    """Load PTTR from the SAME real-downbeat grid the bed uses.
    bar_from is an index into our render bars (0 == DB[_I0])."""
    i = _I0 + bar_from
    j = min(i + n_bars, len(DB) - 1)
    start, end = float(DB[i]), float(DB[j])
    out = {}
    for stem in ("vocals", "other", "bass"):
        y, sr = librosa.load(STEMS / f"{stem}.wav", sr=SR, mono=False,
                             offset=max(0.0, start), duration=end - start)
        out[stem] = (y if y.ndim > 1 else np.stack([y, y])).astype(np.float32)
    return out
...
def build():
    ...
    for bi in play:
        ...
        kick_times.append(slot_at(bi, 0))
        sq.place("kick", slot_at(bi, 0),
                 dagga2(dur=0.44, f0=70, f1=50, amp=1.0, body=0.85,
                        click=0.0, sub_level=0.30))
        if RNG.random() < 0.45:
            kick_times.append(slot_at(bi, 8))
            sq.place("kick", slot_at(bi, 8),
    ...
    span = 22
    S1 = load_bars_db(0, span)
    voc, oth = S1["vocals"], S1["other"]
    steps = span * 8

    def env_for(kind):
        e = np.ones(steps, dtype=np.float32)
        g0, g1 = int(GRACE[0] / span * steps), int(GRACE[1] / span * steps)
        e[g0:g1] = 1.55 if kind == "other" else 0.95
        k = 12
    ...
    put(np.stack([riser(real_bar_len(20) * 2.0, amp=0.30)] * 2), 20, gain=1.0, fin=0.02)

    S2 = load_bars_db(INTRO_BARS, TOTAL - 22)      # back to the hook for the drop
    put(hp(S2["vocals"], 130), 22, gain=1.05)
    put(hp(S2["other"], 240), 22, gain=0.80)
    imp = impact(1.7, amp=0.45)
    s = int(real_bar_time(22) * SR)
...
if __name__ == "__main__":
    ...
    y = build()
    print("wrote", write(str(OUT / "PTTR_jhummar_v20.wav"), y), f"({y.shape[1]/SR:.1f}s)")
